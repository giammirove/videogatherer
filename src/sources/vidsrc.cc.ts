...
async function episode(id: string, type: ItemType = 'tv', s: number = 1, e: number = 1, _server = SERVER_UPCLOUD): Promise<Stream> {
  ...
  const api_url = `https://${HOST}/api/episodes/${new_data_id}/servers?id=${id}&season=${s}&episode=${e}&type=${type}&isMobile=false&v=${encodeURIComponent(v_value)}&vrf=${encodeURIComponent(enc(id))}`;
  debug(ID, api_url);
